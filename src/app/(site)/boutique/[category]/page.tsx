  ...
  return (
    <>
      ...
      <div className="conteneur" style={{ paddingTop: "clamp(28px,3vw,44px)", paddingBottom: "var(--section-y)" }}>
        ...
        <div className="grille grille--boutique" style={{ marginTop: "clamp(24px,3vw,40px)" }}>
          <Filters
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "center", justifyContent: "space-between", marginTop: 24 }}>
              ...
              <form method="get" className="champ" style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                {/* Sans JavaScript, un GET ne renvoie que ses propres champs :
                    les filtres en cours sont réémis pour ne pas être perdus. */}
                {asArray(filters.materiau).map((v) => <input key={`m-${v}`} type="hidden" name="materiau" value={v} />)}
                {asArray(filters.contenance).map((v) => <input key={`c-${v}`} type="hidden" name="contenance" value={v} />)}
                {filters.prixMax ? <input type="hidden" name="prixMax" value={filters.prixMax} /> : null}
                <label htmlFor="tri" style={{ whiteSpace: "nowrap" }}>Trier par</label>
                <SortSelect value={filters.tri} />
