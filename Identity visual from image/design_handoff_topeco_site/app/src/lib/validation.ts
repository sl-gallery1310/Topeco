...
const required = (label: string) => z.string({ required_error: label + " est obligatoire." }).trim().min(1, label + " est obligatoire.");
...
export const appointmentSchema = z.object({
  ...
  consentGiven: z.literal(true, { errorMap: () => ({ message: "Votre accord est nécessaire pour être contacté." }) }),
